...
__global__ void quantgemv(half* mat, half* vec, half* res, unsigned int n,
...
void GEMV2(T* dVecTrans, T* dMatTrans, T* dResTrans, int m, int n, int k) {
  ...
  quantgemv<<<grid_dim, block_dim>>>(dMatTrans, dVecTrans, dResTrans,
                                     vec_height_, mat_height_, num_per_thread);
}

template void GEMV2<half>(half* dVecTrans, half* dMatTrans, half* dResTrans,
                          int m, int n, int k);
